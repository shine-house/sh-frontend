...
import { Edit, Trash, CalendarCheck2, Home, Loader2 } from "lucide-react";
...
const RoomItem: React.FC<RoomItemProps> = ({ room, onSelect }) => {
  ...
  const { zoneCount, weeklyCount, isLoading: isCountingTasks } = useRoomTaskCounts(room.id);
  ...
  return (
    <>
      <Card
        className={cn(
          "relative overflow-hidden cursor-pointer border border-slate-200/60 dark:border-slate-800/80 bg-white dark:bg-slate-900 rounded-2xl transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md hover:border-slate-300 dark:hover:border-slate-700 group",
          isCountingTasks && "pointer-events-none select-none"
        )}
        onClick={() => !isCountingTasks && onSelect(room.id)}
      >
        {/* Conteúdo condicionalmente esmaecido durante o cálculo */}
        <div className={cn("transition-opacity duration-300", isCountingTasks && "opacity-40")}>
        ...
        {/* Spinner centralizado de carregamento local */}
        {isCountingTasks && (
          <div className="absolute inset-0 flex items-center justify-center bg-white/5 dark:bg-slate-900/5 backdrop-blur-[0.5px] animate-in fade-in duration-200">
      ...
      <ConfirmationDialog
        ...
        description={
          totalTasks > 0
            ? `Este cômodo tem ${totalTasks} tarefas associadas. Tem certeza de que deseja excluir? Esta ação não pode ser desfeita.`
            : "Tem certeza de que deseja excluir este cômodo? Esta ação não pode ser desfeita."
        }
